...
void amrReactive(const Box&      a_coarsestDomain,
                 const RealVect& a_dx);
...
/***************/
void amrReactive(const Box&      a_domain,
                 const RealVect& a_dx)
{
  // read inputs
  ParmParse ppgodunov;
  ...
  pout() << "planar reactive shock initial condition" << endl;
  Real shockvel;
  ppgodunov.get("shock_vel",shockvel);

  Real shockcenter;
  ppgodunov.get("shock_center",shockcenter);

  int shocknormal;
  ppgodunov.get("shock_normal",shocknormal);
  ...
  EBPlanarReactiveShockIBCFactory
    bcfactory(shockvel, shockcenter, shocknormal, shockbackward, doRZCoords);

  int iusemassredist;
  ppgodunov.get("use_mass_redist", iusemassredist);
  bool useMassRedist    = (iusemassredist ==1);

  int iaddReactionRates;
  ppgodunov.get("add_reactionRates", iaddReactionRates);
  bool addReactionRates = (iaddReactionRates ==1);

  int iaddDiffusion; 
  ppgodunov.get("add_diffusion", iaddDiffusion);
  bool addDiffusion = (iaddDiffusion ==1);

  //create patch integrator
  EBPatchReactiveFactory patchReactive(&bcfactory,
  ...
  // hasSourceTerm flag is different from useSource
  // useSource - to add raction rates at the end of each timestep

  EBAMRReactiveFactory amrr_fact(initialCFL,
                                 cfl,
                          	 redistRad,
                          	 domainLength,
                          	 refineThresh,
                          	 ...
                          	 &patchReactive);
  AMR amr;
  amr.define(max_level,ref_ratios,prob_domain,&amrr_fact);
  ...
  if (!ppgodunov.contains("restart_file"))
  ...
  else
    {
      ...
      handle.close();
#else
      MayDay::Error("amrReactive restart only defined with hdf5");
#endif
    }
