...
/****************************/
/****************************/
EBPlanarReactiveShockIBC::
EBPlanarReactiveShockIBC(const Real&     a_shockvel,
                         const Real&     a_center,
  ...
  :EBPhysIBC()
{
  ...
  if (a_shockbackward)
    ...
    /**/
  Real preshockpress = 1E05;
  ParmParse pp;
  if (pp.contains("preshockpress"))
    {
      pp.get("preshockpress", preshockpress);
    }
  Real preshocktemp  = 300;
  if (pp.contains("preshocktemp"))
    {
      pp.get("preshocktemp", preshocktemp);
    }
  
  FORT_GET_NSPECIES(CHF_INT(m_nSpecies));
  Vector<Real> initialMassFrac(m_nSpecies);
  Real sumY = 0.;

  for (int ivar = 0; ivar<m_nSpecies-1; ivar++)
   {
     initialMassFrac[ivar] = 1./m_nSpecies;
     sumY = sumY+initialMassFrac[ivar];
   } 
  initialMassFrac[m_nSpecies-1] = 1.-sumY;

  if (pp.contains("initialMassFrac"))
    {
      pp.getarr("initialMassFrac", initialMassFrac,0,m_nSpecies);
    }

  FORT_SETPLANARSHOCK(CHF_CONST_REAL(a_shockvel),
                      CHF_CONST_REAL(a_center),
                      CHF_CONST_REAL(preshockpress),
                      CHF_CONST_REAL(preshocktemp),
                      CHF_CONST_VR(initialMassFrac),
  ...
  vector<Real> cent(SpaceDim);
  pp.getarr("center",cent,0,SpaceDim);

  Real rnot = 1;
  pp.get("rnot",rnot);

  Real small = 1e-8;
  pp.get("smallness",small);

  RealVect center;
  for (int idir = 0; idir < SpaceDim; idir++)
   {
     center[idir] = cent[idir];
   }

/*
  FORT_SETSINEWAVE(CHF_CONST_REALVECT(center),
                   CHF_CONST_REAL(rnot),
                   CHF_CONST_REAL(preshockpress),
                   CHF_CONST_REAL(preshocktemp),
                   CHF_CONST_VR(initialMassFrac),
                   CHF_CONST_REAL(ms),
                   CHF_CONST_REAL(small));
*/
  /**/
...
/****************************/
void
EBPlanarReactiveShockIBC::define(const ProblemDomain&  a_domain,
                         const RealVect& a_dx)
{
  m_domain = a_domain;
  m_dx = a_dx[0];
      ...
      if (a_dir == m_shocknorm && inbackofshock)
        {
          ...
          for (VoFIterator vofit(ivs, a_ebisBox.getEBGraph()); vofit.ok(); ++vofit)
            {
              const VolIndex& vof = vofit();
              Vector<Real> qgdnv(numPrim);
              Vector<Real> fluxv(numFlux);
              ...
              for (int iface= 0; iface < bndryFaces.size(); iface++)
                {
                  ...
                  Real temp = a_primExtrap(vof, QTEMP);
                  Real speed;
                  Vector<Real> massFrac(m_nSpecies);
                  for (int ivar = 0; ivar < m_nSpecies; ivar++)
                   {
                     massFrac[ivar] = a_primExtrap(vof, QSPEC1+ivar);
                   }
                  FORT_GETSOUNDSPEEDTEMP(CHF_CONST_REAL(temp),
                                         CHF_CONST_VR(massFrac),
                                         CHF_REAL(speed));

                  //a_flux[a_dir](face, inormMomVar) = 
                  //  press + isign*dense*unorm*speed;
                  a_flux[a_dir](face, inormMomVar) = press;
                }
            }
        }
    }
