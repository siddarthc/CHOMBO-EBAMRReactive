      SUBROUTINE SOLVEODE(Y,NSPEC,dt,RPAR)

      EXTERNAL FEX
      DOUBLE PRECISION ATOL, RTOL, RWORK, T, TOUT, Y, dt,RPAR
      DIMENSION Y(NSPEC), RWORK(22+9*NSPEC+2*NSPEC**2), IWORK(30+NSPEC)
      DIMENSION RPAR(3)
     ...
     1            IOPT,RWORK,LRW,IWORK,LIW,JEX,MF,RPAR,IPAR)
      ...
      SUBROUTINE FEX (NEQ,TIME,Y,YDOT,RPAR,IPAR)
      DOUBLE PRECISION RPAR,TIME,Y,YDOT,P,T,RHO
      DIMENSION Y(NEQ),RPAR(3),YDOT(NEQ)
      P = RPAR(1)
      T = RPAR(2)
      RHO = RPAR(3)
      CALL DYDT(P,T,RHO,Y,NEQ,YDOT,NEQ,NEQ)
      RETURN
      END
