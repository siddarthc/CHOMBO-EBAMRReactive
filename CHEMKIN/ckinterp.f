...
C     Changes for larger mechanisms (6/3/97 M. Kaneshige
C     1.  Increased IDIM from 500 to 1000
C
...
C     IDIM = maximum number of reactions in a mechanism;        (1000)
...
C
CPAT  STOP (i don't want the fortran stop message)
C      RETURN
      END
C----------------------------------------------------------------------C
...
C
     ...
     1             KCHRG(K)=KCHRG(K)+IELEM*(-1)
            ...
            write(LOUT, *) '>>',ELEM,  MM, M,LINE(1),LINE(1)(ICOL:ICOL+1
     $           ),ICOL
            IF (M .GT. 0) THEN
               KNCF(M,K) = IELEM
