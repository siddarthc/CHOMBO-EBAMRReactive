                                    ...
                                    const int&        a_maxStencilDist)
{
  ...
  m_coarseDomain = coarsen(m_domain, m_refineCoarse);

  m_degree = 3;
  Box stencilBox(-m_maxStencilDist*IntVect::Unit,
                 m_maxStencilDist*IntVect::Unit);
  m_stencils.define(stencilBox, 1);
  for (BoxIterator bit(stencilBox); bit.ok(); ++bit)
    {
      IntVect offset = bit();
      m_stencils(offset, 0) =
        new FourthOrderInterpStencil(offset, m_refineCoarse, m_degree);
    }
