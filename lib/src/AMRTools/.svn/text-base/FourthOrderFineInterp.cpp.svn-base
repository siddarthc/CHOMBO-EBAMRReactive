                                   ...
                                   const int&                a_coarseGhostsFill)
{
  ...
  coarsen(m_layoutCoarsened, m_layout, m_refineCoarse);

  m_maxStencilDist = 2;
  m_patchInterp.define(m_domain, m_refineCoarse, m_maxStencilDist);

  // Now find m_stencilHere:
  // For each coarse cell whose fine cells will be filled in,
  // figure out which stencil to use.
  IntVect coarseGhostVect = m_coarseGhostsFill * IntVect::Unit;
  ...
  m_ghostVect = (m_coarseGhostsFill + m_maxStencilDist) * IntVect::Unit;
