      ...
      if (!ebisBox.isAllCovered())
        {
          ...
          if (m_hasSourceTerm)
            {
              ...
              source.define(ebisBox, bigBox, nCons);
	      ...
	      Real localChi = m_chi;
	      Box here = m_thisGrids.get(dit());
	      int imax = here.bigEnd(0);int imin = here.smallEnd(0);
	      Real xmax = (imax + 0.5)*m_dx[0];
	      Real xmin = (imin + 0.5)*m_dx[0];
	      if(  xmin <= 4) localChi = 0.0; // xmax > 9 ||
	      //int jmin = here.smallEnd(1);
	      //Real ymin = (jmin + 0.5)*m_dx[1];
	      //if (ymin > 1) localChi = 0.0;
// 	      Real factor = 1;
// 	      if (xmax >= 5.9 && xmin < 10.1)
// 		localChi = m_chi;
// 	      else if ( xmax >= 6.1 && xmin < 7.9)
// 		localChi = m_chi/ 2.0;
// 	      else if (xmax >= 8.1 && xmin < 8.9)
// 		localChi = m_chi / (2.0*factor);
// 	      else if (xmax >= 9.1 && xmin < 9.9)
// 		localChi = m_chi / (4.0*factor);
// 	      else
// 		localChi = m_chi / (20.0*factor);
	      Real coec = -localChi; Real coeb = localChi;
	      if (!m_hasSourceTerm) 
		{
		  const Box& bigBox = consState.box();
		  int nCons = m_ebPatchGodunov->numConserved();
		  source.define(ebisBox, bigBox, nCons);
		  source.axby(consState,barState,coec,coeb);
		  //source.setVal(0, 0.0);source.setVal(SpaceDim+1, 0.0);
		  //source.setVal(SpaceDim+1, 0.0);
		}
	      else
		{
...
/*****************************/
//lucaadd
void EBLevelGodunov::getQbar(LevelData<EBCellFAB>*  a_qbar,Real a_chi)
{
  m_SFD = true;
  m_chi = a_chi;
  m_qbar = a_qbar;
}
void EBLevelGodunov::setESource(LevelData<EBCellFAB>*  a_esource)
{
  m_externalSource = true;
  m_esource = a_esource;
}
/*****************************/
/*****************************/
void
EBLevelGodunov::
residual(LevelData<EBCellFAB>&     a_updated,
	 LevelData<EBCellFAB>&      a_consState,
	 LevelData<BaseIVFAB<Real> >&  a_massDiff,
	 EBFluxRegister&               a_fineFluxRegister,
	 EBFluxRegister&               a_coarFluxRegister,
	 ...
	 const Real&                   a_dt)
{
  ...
  EBLevelDataOps::assign(a_updated,a_consState);
...

  {
    CH_TIME("fillConsState");
    //cf interpolation and exchange
    fillConsState(a_updated, a_consStateCoarseOld, a_consStateCoarseNew, a_time, a_coarTimeOld, a_coarTimeNew);
  }
...

  {
    CH_TIME("computeFlattening");
    //compute flattening coefficients. this saves a ghost cell. woo hoo.
    computeFlattening(a_time, a_dt, a_updated);
  }
...

  {
    ...
    //this includes copying flux into flux interpolant and updating
    //regular grids and incrementing flux registers.
    doRegularUpdate(a_fineFluxRegister, a_coarFluxRegister, a_time, a_dt, a_updated);
  }
  ...
  Real maxWaveSpeed;
  {
    ...
    //this does irregular update and deals with flux registers.
    //also computes the mass increment
    maxWaveSpeed = doIrregularUpdate(a_fineFluxRegister, a_coarFluxRegister, a_massDiff, a_time, a_dt, a_updated);
    }

  EBLevelDataOps::incr(a_updated,a_consState,-1.0);
  EBLevelDataOps::scale(a_updated,1.0/a_dt); 
  m_SFD= keep_SFD;
