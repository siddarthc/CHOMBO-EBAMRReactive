...
MixedPoissonDomainBC::MixedPoissonDomainBC()
...
void MixedPoissonDomainBC::setArguments(Vector<Vector<int> >&  a_domMixBc, Real a_dvalue, Real a_nvalue, RefCountedPtr<BaseMixBCValue> a_func)
{
...
void MixedPoissonDomainBC::setEBOrder(int a_ebOrder)
{
...
*/
void MixedPoissonDomainBC::getFaceFlux(BaseFab<Real>&        a_faceFlux,
...
*/
void MixedPoissonDomainBC::getFaceFlux(Real&                     a_faceFlux,
  ...
  if (faces.size() > 0)
    {
      if (faces.size()==1)
        ...
        {
          MayDay::Error("MixedPoissonDomainBC::getHigherOrderFaceFlux has multi-valued faces (or could be 0 faces)");
        }
    }
}
...
*/
void MixedPoissonDomainBC::getFaceGradPhi(Real&                     a_faceFlux,
					  const FaceIndex&      a_face,
					  const int&            a_comp,
					  const EBCellFAB&      a_phi,
					  const RealVect&       a_probLo,
...
*/
void MixedPoissonDomainBC::getFaceFluxGradPhi(Real&                       a_faceFlux,
					      const FaceIndex&      a_face,
					      const int&            a_comp,
					      const EBCellFAB&      a_phi,
					      const RealVect&       a_probLo,
...
/****************
   ...
   For domain boundary conditions on pressure in projections.
*/
void MixedPoissonDomainBC::getInhomFaceFlux(Real&                     a_faceFlux,
					    const VolIndex&       a_vof,
					    const int&            a_comp,
					    const EBCellFAB&      a_phi,
					    const RealVect&       a_probLo,
...
/****************
   ...
   but also by macEnforceGradientBC which needs the whole stencil (can stencilize this later)
*/
void MixedPoissonDomainBC::getInhomFaceGradPhi(Real&                     a_faceFlux,
					       const FaceIndex&      a_face,
					       const int&            a_comp,
					       const EBCellFAB&      a_phi,
					       const RealVect&       a_probLo,
 ...
 */
void MixedPoissonDomainBC::getInhomFaceFluxGradPhi(Real&                 a_faceFlux,
						   const FaceIndex&      a_face,
						   const int&            a_comp,
						   const EBCellFAB&      a_phi,
						   const RealVect&       a_probLo,
...
/****************
 // From Dirichlet:
 */
void MixedPoissonDomainBC::getFluxStencil(VoFStencil&            a_stencil,
					  const VolIndex&        a_vof,
					  const int&             a_comp,
					  const RealVect&        a_dx,
					  const int&             a_idir,
  ...
  if (faces.size() > 0)
    {
	  ...
	  //end of LM changes
        ...
        {
          MayDay::Error("MixedPoissonDomainBC::getFluxStencil has multi-valued faces");
        }
    }
					  ...
					  const EBISBox&         a_ebisBox)
{
  ...
  if (m_ebOrder == 1)//pressure
    ...
    {
      MayDay::Error("MixedPoissonDomainBC::getFluxStencil -- bad BC order");
    }
}

void MixedPoissonDomainBC::getFirstOrderFluxStencil(      VoFStencil&      a_stencil,
							  const FaceIndex&       a_face,
							  const int&             a_comp,
							  const RealVect&        a_dx,
							  const int&             a_idir,
...
void MixedPoissonDomainBC::getSecondOrderFluxStencil(      VoFStencil&      a_stencil,
							   const FaceIndex&       a_face,
							   const int&             a_comp,
							   const RealVect&        a_dx,
							   const int&             a_idir,
...
/****************
 // Factory Routines:
 */
MixedPoissonDomainBCFactory::MixedPoissonDomainBCFactory()
  : m_dvalue(12345.6789),
    m_nvalue(12345.6789),
    m_func(RefCountedPtr<BaseMixBCValue>()),
    m_ebOrder(2),
...
void MixedPoissonDomainBCFactory::setArguments(Vector<Vector<int> >& a_domMixBc, Real a_dvalue, Real a_nvalue, RefCountedPtr<BaseMixBCValue> a_func)
{
  m_domMixBc = a_domMixBc;
  m_dvalue = a_dvalue;
  m_nvalue = a_nvalue;
...
void MixedPoissonDomainBCFactory::setEBOrder(int a_ebOrder)
{
  CH_assert(a_ebOrder >= 1);
  CH_assert(a_ebOrder <= 2);
...
void MixedPoissonDomainBCFactory::setTime(Real* a_time)
{
  m_timePtr = a_time;
}
...
create(const ProblemDomain& a_domain,
       ...
       const RealVect&      a_dx)
{
  MixedPoissonDomainBC* newBC = new MixedPoissonDomainBC();
