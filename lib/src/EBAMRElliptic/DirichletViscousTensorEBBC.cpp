  ...
  for (a_vofit.reset(); a_vofit.ok(); ++a_vofit)
    {
      ...
      for (int comp = 0; comp < SpaceDim; comp++)
        {
          a_lphi(vof, comp) += (deltaLph[comp]*a_factor);
        }
    }
}
            ...
            bool a_homogeneous)
{
  if (m_isFunction && (!a_homogeneous))
  ...
  else
    {
      ...
      getTangentVectors(tangents, normal);
  ...
  for (int ivar = 0; ivar < SpaceDim; ivar++)
    {
      for (int idir = 0; idir < SpaceDim; idir++)
        {
          gradCart[ivar][idir] = m_func->derivative(point, normal, ivar, idir);
        }
    }
  ...
  for (int ivar = 0; ivar < SpaceDim; ivar++)
    {
      normalStencil.setAllVariables(ivar);
      Real normalGrad = applyVoFStencil(normalStencil, a_phi, ivar);
      Real value = m_func->value(point, normal, ivar);
