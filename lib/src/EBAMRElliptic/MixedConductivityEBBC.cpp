...
#include "MixedConductivityEBBC.H"
#include "CH_Timer.H"
...
int MixedConductivityEBBC::s_velComp = 0;

bool MixedConductivityEBBC::s_areaFracWeighted = false;
Real MixedConductivityEBBC::s_timePrint = 0;

MixedConductivityEBBC::MixedConductivityEBBC()
{
  m_dataBased = false;
}

MixedConductivityEBBC::MixedConductivityEBBC(const ProblemDomain& a_domain,
...
void
MixedConductivityEBBC::construct(const ProblemDomain& a_domain,
			    const EBISLayout&    a_layout,
			    const RealVect&      a_dx,
			    const IntVect*       a_ghostCellsPhi /*=0*/,
			    const IntVect*       a_ghostCellsRhs /*=0*/)
...
void MixedConductivityEBBC::setArguments(Real 							a_dvalue, 
				    Real 							a_nvalue, 
...
void MixedConductivityEBBC::setOrder(int a_order)
{
...
void MixedConductivityEBBC::define(const LayoutData<IntVectSet>& a_cfivs,
                              const Real&                   a_factor)
{
  bool isDirBC;
  if ((m_ebType == 1) || (m_ebType == 10))
      ...
      for (DataIterator dit = dbl.dataIterator(); dit.ok(); ++dit)
	{
	  ...
	  for (VoFIterator vofit(notRegular,curEBGraph); vofit.ok(); ++vofit)
		...
		{
		  MayDay::Error("MixedConductivityEBBC::define stencil order not 1 or 2");
		}
	      //Need to magnify weight in fluxStencil with areafrac*factor, factor = 1/dx;
	      //Pass the factor in because m_dx[0] in here may not be same as in EBAMRConductivityOp
...
void MixedConductivityEBBC::defineCustom(const LayoutData<IntVectSet>& a_cfivs,
				    const Real&                   a_factor)
{
		  ...
		  if (curBox.contains(ivLo))
		      ...
		      else
			{
			  ...
			  if (curBox.contains(ivHi))
				...
				{
				  pout() << "MixedConductivityEBBC::defineCustom, vofHi/Lo indexes outside of irregular IVS " << ivLo << ivHi << endl;
...
LayoutData<BaseIVFAB<VoFStencil> >* MixedConductivityEBBC::getFluxStencil(int ivar)
{
  if ((m_ebType == 1) || (m_ebType == 10))
    {
...
void MixedConductivityEBBC::applyEBFlux(EBCellFAB&                    a_lphi,
				   ...
				   const Real&                   a_time)
{
  CH_TIME("MixedConductivityEBBC::applyEBFlux");
  ...
  for (a_vofit.reset(); a_vofit.ok(); ++a_vofit)
    {
	  ...
	  if (isDirBC)
		...
		{ 
		  ...
		  //hardwired source parameter from conductivity.inputs now, to be passed from higher level array later on				
      ...
      else
        {
          if (m_onlyHomogeneous)
            {
              MayDay::Error("MixedConductivityEBBC::applyEBFlux called with undefined inhomogeneous BC");
            }
	  else if (m_ebType == 1)
	    {
	      Real value;
  ...
  /*if(abs(buffer)>0 && time > s_timePrint){
    pout() << "MixedConductivityEBBC::applyEBFlux, m_timePtr=" << time << ", dirichlet value on EB electrode =" << buffer << endl;s_timePrint=time;}*/
...
void MixedConductivityEBBC::getFirstOrderStencil(VoFStencil&     a_stencil,
					    Real&           a_weight,
					    const VolIndex& a_vof,
					    const EBISBox&  a_ebisBox,
					    const RealVect& a_dx)
{
  CH_TIME("MixedConductivityEBBC::getFirstOrderStencil1");
  EBArith::getLeastSquaresGradSten(a_stencil, a_weight, a_vof, a_ebisBox, a_dx, m_domain);
}

void MixedConductivityEBBC::getSecondOrderStencil(VoFStencil&       a_stencil,
					     Real&             a_weight,
					     const VolIndex&   a_vof,
					     const EBISBox&    a_ebisBox,
					     const RealVect&   a_dx,
...
MixedConductivityEBBCFactory::MixedConductivityEBBCFactory()
{
  m_dvalue = 12345.6789;
  m_nvalue = 12345.6789;
  m_func = RefCountedPtr<BaseMixBCValue>();
...
void MixedConductivityEBBCFactory::setOrder(int a_order)
{
  CH_assert(a_order >= 1 && a_order <= 2);
  m_order = a_order;
}

void MixedConductivityEBBCFactory::setArguments(Real a_dvalue, Real a_nvalue, RefCountedPtr<BaseMixBCValue> a_func, int a_ebType, RealVect a_dymin, Real a_epsLo, Real a_epsHi)
{
  m_dvalue = a_dvalue;
  m_nvalue = a_nvalue;
  m_func = a_func;
...
void MixedConductivityEBBCFactory::setTime(Real* a_time)
{
  m_timePtr = a_time;
}

MixedConductivityEBBC* MixedConductivityEBBCFactory::create(const ProblemDomain& a_domain,
						  const EBISLayout&    a_layout,
						  const RealVect&      a_dx,
						  const IntVect*       a_ghostCellsPhi /*=0*/,
						  const IntVect*       a_ghostCellsRhs /*=0*/)
{
  CH_TIME("MixedConductivityEBBC::create");

  MixedConductivityEBBC* fresh = new MixedConductivityEBBC(a_domain,a_layout,a_dx,a_ghostCellsPhi,
						 a_ghostCellsRhs);
...
/******************************************/
void MixedConductivityEBBC::setEBBCSource(const LevelData<BaseIVFAB<Real> >&     a_rhos)
{
		...
		{			
		  ...
		  if (curBox.contains(ivLo))
			...
			{//Low side; find the Hi Side
			  const IntVect& ivHi = iv + signVof*BASISV(1);
			  if (!notRegular.contains(ivHi)) pout() << vof <<  " MixedConductivityEBBC::setEBBCSource (LM) messed up (it will probably crash) " << endl;
