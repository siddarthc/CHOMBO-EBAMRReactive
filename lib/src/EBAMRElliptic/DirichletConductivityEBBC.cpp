...
//            }
        ...
        {
          RealVect point = ebisBox.bndryCentroid(vof);
	  point *= a_dx;
	  point += EBArith::getVofLocation(vof,a_dx,a_probLo);
          const RealVect&   normal = ebisBox.normal(vof);

          value = m_bc.m_func->value(point,normal,a_time,s_velComp);
        }
      else
        {
...
/*****************/
DirichletConductivityEBBCFactory::DirichletConductivityEBBCFactory()
{
  m_value = 12345.6789;
