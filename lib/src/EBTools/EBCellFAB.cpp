              ...
              for (int ivof = 0; ivof < vofs.size(); ivof++)
                {
                  ...
                  for (int icomp = 0; icomp < a_numcomp; ++icomp)
                    {
                      m_irrFAB(vof, a_destcomp+icomp) +=
                        a_src.m_irrFAB(vof, a_srccomp+icomp) * a_scale;
                    }
                }
            }
        }
...
{
  Box locRegion = a_src.m_region & m_region;
  plus(a_src, locRegion, a_srccomp, a_destcomp, a_numcomp, a_scale);
  return *this;
}
/**********************/
/**********************/
                ...
                int a_numcomp)
{
  ...
  FORT_MULTIPLYFABRCOMP(CHF_FRA(m_regFAB),
...
/**********************/
/**********************/
void EBCellFAB::powF(const Real& a_exp)
{
  CH_assert(isDefined());

  if (!m_region.isEmpty())
    {
      FORT_POWFAB(CHF_FRA(m_regFAB),
		  CHF_CONST_REAL(a_exp),
		  CHF_BOX(m_region),
		  CHF_INT(m_nComp));

      Real* c = m_irrFAB.dataPtr(0);
      int nvof = m_irrFAB.numVoFs();
      for (int i=0; i<m_nComp*nvof; i++)
	c[i] = pow(c[i] , a_exp);
    }
...
// lucaadd
EBCellFAB&
EBCellFAB::arrayNorm(const EBCellFAB& a_src, Real a_scale)
{
  CH_assert(isDefined());
  CH_assert(a_src.isDefined());
  Box locRegion = a_src.m_region & m_region;
  bool sameRegBox = (a_src.m_regFAB.box() == m_regFAB.box());
  int destcomp =0;
  int mComps = a_src.m_nComp;
  if (!locRegion.isEmpty())
    {
      FORT_ARRAYNORM(CHF_FRA(m_regFAB),
		     CHF_CONST_FRA(a_src.m_regFAB),
		     CHF_CONST_REAL(a_scale),
		     CHF_BOX(locRegion));
	  ...
	  int srccomp=0;
          Real* l = m_irrFAB.dataPtr(destcomp);
	  const Real* r = a_src.m_irrFAB.dataPtr(srccomp);
	  for (int i=0; i < nvof; i++) l[i] = r[i] * r[i];
	  for (int srccomp=1; srccomp< mComps; srccomp++)
	    {
	      const Real* r = a_src.m_irrFAB.dataPtr(srccomp);
	      for (int i=0; i < nvof; i++) l[i] += r[i] * r[i];
	    }
	  for (int i=0; i < nvof; i++) l[i] *= a_scale;
        }
      else
        {
          IntVectSet ivsMulti = a_src.getMultiCells();
          ...
          for (ivsit.reset(); ivsit.ok(); ++ivsit)
            {
              ...
              for (int ivof = 0; ivof < vofs.size(); ivof++)
                {
                  const VolIndex& vof = vofs[ivof];
		  m_irrFAB(vof, destcomp) = a_src.m_irrFAB(vof, 0) * a_src.m_irrFAB(vof, 0);
                  for (int icomp = 1; icomp < mComps; ++icomp)
                    {
                      m_irrFAB(vof, destcomp) += a_src.m_irrFAB(vof, icomp) * a_src.m_irrFAB(vof, icomp);
                    }
		  m_irrFAB(vof, destcomp) *= a_scale;
                }
            }
        }
    }
